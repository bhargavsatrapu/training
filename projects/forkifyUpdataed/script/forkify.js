...
;     
//${currentRecipe.data.recipe.ingredients[k].quantity+currentRecipe.data.recipe[k].ingredients.unit+currentRecipe.data.recipe.ingredients[k].description}
...
async function showContent(ak) {
    ingredientList.innerHTML = "";

    var response1 = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes/${ak}?key=<insert your key>`);
    currentRecipe = await response1.json();
    console.log(currentRecipe);
    n=currentRecipe.data.recipe.servings;
    calculation();
}
...
async function json1() {
    ...
    console.log(key);
    var key = document.querySelector(".search-box").value;

    var response = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes?search=${key}&key=442002e6-23dc-4e1c-a9d7-aaebfb9a279e`);

    jData = await response.json();
    console.log(jData);
    console.log(jData.data.recipes.length);

    if (jData.data.recipes.length === 0) {
        error1.style.display = "block";
    } else if (jData.data.recipes.length < pageSize) {
